...
create_dbwin (char *title, int width, int height)
{
  ...
  dbwin = gtk_notebook_new ();
  gtk_object_set_data (GTK_OBJECT (dbwin), "dbwin", dbwin);
  vertholder = gtk_vbox_new (FALSE, 0);
  gtk_widget_ref (vertholder);
  DPRINT("hello world\n");

  gtk_object_set_data_full (GTK_OBJECT (dbwin), "vertholder",
			    vertholder, (GtkDestroyNotify) gtk_widget_unref);

  gtk_widget_show (vertholder);
  gtk_container_add (GTK_CONTAINER (dbwin), vertholder);

  gtk_widget_set_size_request(dbwin,300,300);
  DPRINT("Creating dbobject\n");
  dbobject = gtk_scrolled_window_new (NULL, NULL);

  
 gtk_widget_set_size_request(dbobject,900,500);
 

 DPRINT("Referencing\n");
  gtk_widget_ref (dbobject);


  gtk_object_set_data_full (GTK_OBJECT (dbwin),
			    "dbobject", dbobject,
			    (GtkDestroyNotify) gtk_widget_unref);
  gtk_widget_show (dbobject);

  DPRINT("dbobject show\n");
  gtk_box_pack_start (GTK_BOX (vertholder), dbobject, TRUE, TRUE, 0);
  ...
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (dbobject),
...
void
create_frontdisplay (char *dbtitle,char *customerlogo, GtkWidget * localwindow,int width,int height)
{
  int counter=0;	
  GtkWidget *image;
  GtkWidget *hbox;
  ...
  create_dbwin (dbtitle,(width*5)/6,height);


  DPRINT("Before starting vbox\n");
  ...
  gtk_box_pack_start (GTK_BOX (hbox), dbwin, FALSE, FALSE, 0);
		   ...
		   int width, int height)
{
  ...
  gtk_window_set_title (GTK_WINDOW (mainwindow), prodname);
  ...
  menubar = get_menubar_menu (mainwindow);
  toolbar = get_toolbar (mainwindow);
  DPRINT("Creating main window success\n");
